#!/usr/bin/env python3
"""Send PKT_ESTOP and expect PKT_FAULT(-2001)."""
...
def build_raw(pkt_type: int, seq: int) -> bytes:
...
def parse_fault(raw: bytes) -> tuple[int, int, int]:
    ...
    code = struct.unpack_from("<i", raw, 8)[0] if plen >= 4 else 0
    return typ, seq, code
...
def main() -> None:
    ...
    ap.add_argument("--seq", type=int, default=88)
    args = ap.parse_args()

    port = args.port or (sorted(glob.glob("/dev/cu.usbmodem*"))[-1])
    tx = cobs_encode(build_raw(PKT_ESTOP, args.seq))
    ...
    with serial.Serial(port, args.baud, timeout=0.5) as ser:
        ...
        ser.write(tx)
        ser.flush()
        print(f"Sent PKT_ESTOP seq={args.seq}")

        deadline = time.time() + 3.0
        while time.time() < deadline:
            frame = read_frame(ser, timeout=1.0)
            try:
                typ, seq, code = parse_fault(frame)
            except ValueError:
                continue
            if typ != PKT_FAULT:
                continue
            if seq != args.seq:
                continue
            print(f"OK: fault seq={seq}, code={code}")
            return

    print("FAIL: no PKT_FAULT", file=sys.stderr)
    raise SystemExit(1)
